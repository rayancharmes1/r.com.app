...
export default function BoutiquesPage() {
  ...
  useEffect(() => {
    ...
    return onValue(r, snap => {
      ...
      const data = snap.val();
      const fbMap = {};
      Object.values(data).forEach(v => { if (v.isDefault && v.id) fbMap[v.id] = v; });
      const merged = DEFAULTS.map(d => ({ ...d, ...(fbMap[d.id] || {}) }));
      setDisciplines(merged.filter(d => d.available));
    });
  }, []);
        ...
        <>
          {disciplines.length > 0 && (
            <>
              ...
              <div style={s.grid}>
                {disciplines.map(d => (
                  <Link key={d.id} to={`/shop/${d.id}`} style={{ ...s.card, background: t.cardBg, color: t.text, borderLeft: `4px solid ${d.color}` }}>
                    <div style={{ ...s.icon, background: `linear-gradient(135deg, ${d.color}, ${d.color}aa)` }}>{d.icon}</div>
                    <div>
                      <h2 style={s.name}>{d.name}</h2>
                      {d.description && <p style={{ ...s.desc, color: t.sub }}>{d.description}</p>}
                    </div>
